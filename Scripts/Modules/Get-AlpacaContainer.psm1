function Get-AlpacaContainer {
    ...
    try {
        ...
        Write-AlpacaDebug "Got $($response.Count) containers. Ids: $($response | ForEach-Object { $_.id } | ConvertTo-Json -Compress)"
        ...
        Write-AlpacaDebug "Filtered down to $($containers.Count) container(s) based on project and build mode filter: $($containers | Select-Object Id,Project,BuildMode | ConvertTo-Json -Compress)"
        Write-AlpacaOutput "Returning $($containers.Count) container(s): $($containers | ForEach-Object { $_.id } | Join-String -Separator ", ")"
        return $containers
    }
    finally {
