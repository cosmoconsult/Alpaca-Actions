...
function Resolve-ReviewToken {
    [Diagnostics.CodeAnalysis.SuppressMessageAttribute('PSReviewUnusedParameter', 'Repository', Justification = 'Used inside scriptblock')]
    param(
        [Parameter(Mandatory = $true)]
        [string] $Repository,
        [Parameter(Mandatory = $true)]
    ...
    $decodedGitHubToken = [System.Text.Encoding]::UTF8.GetString([System.Convert]::FromBase64String($GitHubToken))
    return [pscustomobject]@{
        token       = Invoke-ALGoCommand -ScriptBlock { GetAccessToken -token $decodedGitHubToken -repository $Repository -permissions @{ "contents" = "read"; "pull_requests" = "write" } }
        tokenSource = 'ghTokenWorkflow'
    }
}
...
function Resolve-FindingPath {
    ...
    $suffixMatches = @($ChangedFiles | Where-Object { $_.EndsWith("/$normalizedFindingPath", [System.StringComparison]::OrdinalIgnoreCase) -or $normalizedFindingPath.EndsWith("/$_", [System.StringComparison]::OrdinalIgnoreCase) })
...
function Get-DiagnosticCodeUrl {
    ...
    switch -Regex ($RuleId) {
        ...
        '^PC(?<ruleId>\d{4})$' { return ('https://alcops.dev/docs/analyzers/platformcop/pc{0}' -f $matches['ruleId']) }
