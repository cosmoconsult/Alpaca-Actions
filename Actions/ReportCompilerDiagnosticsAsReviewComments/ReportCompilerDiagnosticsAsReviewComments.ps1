...
try {
    ...
    $reviewHashPattern = ("(?i)$magicHashMarkerTemplate" -f '(?<hash>[a-f0-9]{16})')
    $markedReviews = @($reviews | Where-Object { $_.body -match $reviewHashPattern })
    $matchingReviews = @($markedReviews | Where-Object {
            $reviewHashMatch = [regex]::Match($_.body, $reviewHashPattern)
            $reviewHashMatch.Success -and $reviewHashMatch.Groups['hash'].Value.Equals($newReviewHash, [System.StringComparison]::OrdinalIgnoreCase)
        })
    $currentReview = $matchingReviews | Select-Object -Last 1
    $reviewMatches = $null -ne $currentReview

    Write-AlpacaOutput -Message "Found $($markedReviews.Count) marked compiler diagnostics review(s)."
    Write-AlpacaDebug -Message "Expected review hash: $newReviewHash"
    Write-AlpacaDebug -Message "Current review: $($currentReview.html_url)"
    Write-AlpacaDebug -Message "Review matches: $reviewMatches"

    if (-not $reviewMatches -and $relevantFindings.Count -gt 0) {
        ...
        # Refresh reviews so the newly created review is retained and all older marked reviews can be removed.
        $reviews = @(Invoke-GhApiJson -Arguments @('api', "/repos/$Repository/pulls/$PullRequestNumber/reviews?per_page=100") -Paginate)
        $markedReviews = @($reviews | Where-Object { $_.body -match $reviewHashPattern })
        $matchingReviews = @($markedReviews | Where-Object {
                $reviewHashMatch = [regex]::Match($_.body, $reviewHashPattern)
    ...
    $reviewsToRemove = @($markedReviews | Where-Object { $null -eq $currentReview -or $_.id -ne $currentReview.id })
    foreach ($reviewToRemove in $reviewsToRemove) {
        Write-AlpacaDebug -Message "Fetching comments for compiler diagnostics review $($reviewToRemove.id)."
        $existingReviewComments = @(Invoke-GhApiJson -Arguments @('api', "/repos/$Repository/pulls/$PullRequestNumber/reviews/$($reviewToRemove.id)/comments?per_page=100"))
        Write-AlpacaOutput -Message "Removing $($existingReviewComments.Count) comments from previous compiler diagnostics review $($reviewToRemove.id)."
        ...
        Write-AlpacaOutput -Message "Hiding submitted compiler diagnostics review $($reviewToRemove.id) as outdated."
        Invoke-GhCommentMinimize -SubjectNodeId $reviewToRemove.node_id
    }

    if ($reviewMatches) {
        Write-AlpacaOutput -Message "Existing compiler diagnostics review and comments match. No new review was required."
    }
}
catch {
